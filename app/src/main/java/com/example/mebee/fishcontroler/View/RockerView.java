package com.example.mebee.fishcontroler.View;
...
import com.example.mebee.fishcontroler.R;
...
public class RockerView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        mAreaRadius = (int) (cy / (mRockerScale + 1));
        ...
        if (AREA_BACKGROUND_MODE_PIC == mAreaBackgroundMode || AREA_BACKGROUND_MODE_XML == mAreaBackgroundMode) {
            ...
            //Rect dst = new Rect(mCenterPoint.x - 5, mCenterPoint.y - mAreaRadius, mCenterPoint.x + 5, mCenterPoint.y + mAreaRadius);
    ...
    private Point getRockerPositionPoint(Point centerPoint, Point touchPoint, float regionRadius, float rockerRadius) {
        ...
        if (lenXY + rockerRadius <= regionRadius) { // 触摸位置在可活动范围内
            // 回调 返回参数
            callBack(angle, (int) lenXY,lenX,lenY);
            return touchPoint;
        } else { // 触摸位置在可活动范围以外
            // 计算要显示的位置
     ...
     * @param angle 摇动角度
     */
    private void callBack(double angle, float distance,float lenX,float lenY) {
            ...
            if (CallBackMode.CALL_BACK_MODE_MOVE == mCallBackMode) {
                ...
                switch (mDirectionMode) {
                    ...
                    case DIRECTION_2_VERTICAL:// 上下方向
                       ...
                       // Log.d(TAG, "mAreaRadius: "+mAreaRadius+"---"+"lenY："+lenY);

                        /*if ( Math.abs(lenY) < mAreaRadius/2) {
                            ...
                            if (ANGLE_VERTICAL_2D_OF_1P <= angle && ANGLE_360 > angle && tempDirection != Direction.DIRECTION_UP_MORE) {
                                // 上上
                                tempDirection = Direction.DIRECTION_UP_MORE;
                                mOnShakeListener.direction(Direction.DIRECTION_UP_MORE);
                            } else if (ANGLE_VERTICAL_2D_OF_0P <= angle && ANGLE_VERTICAL_2D_OF_1P > angle && tempDirection != Direction.DIRECTION_DOWN_MORE) {
                                // 下下
                                tempDirection = Direction.DIRECTION_DOWN_MORE;
                                mOnShakeListener.direction(Direction.DIRECTION_DOWN_MORE);
                            }
                        }*/
                        if (ANGLE_VERTICAL_2D_OF_0P <= angle && ANGLE_VERTICAL_2D_OF_1P > angle && tempDirection != Direction.DIRECTION_DOWN) {
                            // 下
    ...
    public enum Direction {
        ...
        DIRECTION_DOWN_MORE, DIRECTION_UP_MORE, DIRECTION_CENTER // 中间
    }
